...
class Recurrent_block(nn.Module):
    def __init__(self, ch_out, t=2, kernel_size=3, dilation=1, groups=8):
        super(Recurrent_block, self).__init__()
        self.t = t
        self.conv = nn.Sequential(
            nn.Conv2d(ch_out, ch_out, kernel_size=kernel_size, stride=1, padding=1, dilation=dilation, groups=groups, bias=True),
            nn.BatchNorm2d(ch_out),
            nn.ReLU(inplace=True)
        )
...
class RRCNN_block(nn.Module):
    def __init__(self, ch_in, ch_out, t=2, kernel_size=3, dilation=1, groups=8):
        super(RRCNN_block, self).__init__()
        self.RCNN = nn.Sequential(
            Recurrent_block(ch_out, t=t, kernel_size=kernel_size, dilation=dilation, groups=groups),
            Recurrent_block(ch_out, t=t, kernel_size=kernel_size, dilation=dilation, groups=groups)
        )
