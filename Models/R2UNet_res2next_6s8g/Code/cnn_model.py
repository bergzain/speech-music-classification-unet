...
class Res2NextBlock(nn.Module):
    def __init__(self, in_channels, out_channels, scales=6, groups=8):
        ...
        for _ in range(scales):
            self.convs.append(nn.Conv2d(in_channels, self.width, kernel_size=3, padding=1, groups=groups))
...
class Recurrent_block(nn.Module):
    def __init__(self, ch_out, t=2):
        ...
        self.t = t
        self.res2next_block = Res2NextBlock(ch_out, ch_out)

    def forward(self, x):
        x1 = None
        for i in range(self.t):
            if i == 0:
                x1 = self.res2next_block(x)
            else:
                x1 = self.res2next_block(x + x1)
        return x1
