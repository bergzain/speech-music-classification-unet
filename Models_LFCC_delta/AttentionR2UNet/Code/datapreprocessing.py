...
class AudioProcessor(Dataset):
    ...
    def preprocess(self, filepath):
        ...
        lfcc_deltas = []
        for wf in waveforms:
            # Compute LFCCs for each waveform chunk
            lfcc = torchaudio.transforms.LFCC(sample_rate=self.target_sample_rate,
                                            n_lfcc=self.n_mfcc)(wf)
            
            # computer the first derivative of LFCCs
            lfcc_delta = torchaudio.transforms.ComputeDeltas()(lfcc)
            # Reshape lfcc to [1, 32, 1120]
            lfcc_delta = lfcc_delta[:, :, :1120] 
            if lfcc_delta.shape[2] < 1120:
                # If the resulting lfcc_delta is too short in the time dimension, pad it
                lfcc_delta = torch.nn.functional.pad(lfcc_delta, (0, 1120 - lfcc_delta.shape[2]))
            lfcc_deltas.append(lfcc_delta)
        return torch.stack(lfcc_deltas)
