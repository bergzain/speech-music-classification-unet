...
class AudioProcessor(Dataset):
    ...
    def preprocess(self, filepath):
        ...
        for wf in waveforms:
            ...
            # Reshape delta to [1, 32, 1120]
            delta = delta[:, :, :1120] 
            if delta.shape[2] < 1120:
                # If the resulting delta features are too short in the time dimension, pad them
                delta = torch.nn.functional.pad(delta, (0, 1120 - delta.shape[2]))
            deltas.append(delta)
