...
class AudioProcessor(Dataset):
    ...
    def preprocess(self, filepath):
        ...
        for wf in waveforms:
            ...
            lfcc = lfcc[:, :, :1120] 
            if lfcc.shape[2] < 1120:
                # If the resulting LFCC is too short in the time dimension, pad it
                lfcc = torch.nn.functional.pad(lfcc, (0, 1120 - lfcc.shape[2]))
            lfccs.append(lfcc)
        return torch.stack(lfccs)
